...
- ✨ ذكاء Gemini: خانة API Key في تاب حسابي (محلي على جهازك فقط — لا سحابة ولا نسخ) + زرار ✨ في المودال يحسّن المهمة (عنوان/وصف/فرعية/أولوية) للمراجعة قبل الحفظ — هات المفتاح مجاناً من aistudio.google.com (الموديل قابل للتغيير من نفس الخانة — الحالي gemini-3.6-flash)
- 🤖 مساعد عائم: زر على أي موقع يفتح شات Gemini بمهامك الحية (تذكير/تشجيع/إضافة مهام بالأمر) + تشغيل/إيقاف من ⚙️ (يطلب إذن المواقع مرة واحدة، بدون تحذير تسطيب) + سحب الزر لأي مكان + 8 أيقونات و6 ألوان وشكلين من التخصيص + ⚙️ تخصيص الشات نفسه (خطوط عربية/حجم/فاتح-داكن-مخصص/ألوان الخلفية والرسائل) مع وضوح أعلى (اتجاه تلقائي وتباعد مريح)
